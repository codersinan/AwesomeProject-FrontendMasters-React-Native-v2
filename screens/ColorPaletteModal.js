...
const ColorPaletteModal = ({ navigation }) => {
    ...
    const handleValueChange = useCallback((value, color) => {
        if (value === true) {
            ...
            setSelectedColors(colors => colors.filter(selectedColor => selectedColor.colorName !== color.colorName))
        }
    }, []);
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <View style={styles.color}>
                        ...
                        <Switch value={!!selectedColors.find(color => color.colorName === item.colorName)} onValueChange={(selected) => { handleValueChange(selected, item) }}></Switch>
                    </View>
                )}
            />
